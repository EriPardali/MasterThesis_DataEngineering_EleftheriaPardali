...
# Inside Airflow container (volume-mounted data folder)
IN_PATH = Path("/opt/airflow/data/processed/loan_kpi_staging.csv")
OUT_PATH = Path("/opt/airflow/data/processed/loan_kpi_staging_typed.csv")
...
def main():
    if not IN_PATH.exists():
        raise FileNotFoundError(f"Input CSV not found inside container: {IN_PATH}")
    ...
    int_cols_small = [
        "issue_d_year", "issue_d_month", "emp_length_years",
        "grade_ord", "sub_grade_ord", "loan_status_binary"
    ]
